package com.hhmd.videocrop.utils;
...
public class CameraUtils
{
    ...
    public static void pickAudioFromSystem(Activity activity)
    {
        ...
        activity.startActivityForResult(intent,
                RequestCode.FLAG_REQUEST_SYSTEM_AUDIO);
    }
    ...
    public static void pickVideoFromSystem(Activity activity) {
        ...
        /**
         * 参数一:打开系统视频的ACTION 参数二:返回数据的方式(从系统相册的数据库获取)
         */
        Intent intent = new Intent(Intent.ACTION_PICK,
                MediaStore.Video.Media.EXTERNAL_CONTENT_URI);
        activity.startActivityForResult(intent,
                RequestCode.FLAG_REQUEST_SYSTEM_VIDEO);
    }
    ...
    public static String getVideoPathFromSystem(Activity activity, Intent data){
        ...
        {
            c.moveToFirst();
            // 通过游标来获取名为MediaStore.Video.Media.DATA字段的值
            String path = c.getString(c
    ...
    public static void openAudioRecordForAudio(Activity activity)
    {
        ...
        activity.startActivityForResult(intent,
                RequestCode.FLAG_REQUEST_RECORD_AUDIO);
    }
...
//        intent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(new File(CAMERA_VIDEO)));
    ...
    {
        int FLAG_REQUEST_SYSTEM_AUDIO = 0;

        int FLAG_REQUEST_SYSTEM_IMAGE = 1;

        int FLAG_REQUEST_SYSTEM_VIDEO = 2;

        int FLAG_REQUEST_CAMERA_IMAGE = 3;

        int FLAG_REQUEST_RECORD_AUDIO = 4;
