...
import com.hhmd.videocrop.cropview.window.CropVideoView;
import com.hhmd.videocrop.cropview.window.CropView;
import com.hhmd.videocrop.ffmpeg.ExecuteBinaryResponseHandler;
import com.hhmd.videocrop.ffmpeg.FFmpeg;
import com.hhmd.videocrop.ffmpeg.FFtask;
import com.hhmd.videocrop.player.VideoPlayer;
import com.hhmd.videocrop.view.ProgressView;
import com.hhmd.videocrop.view.VideoSliceSeekBarH;
...
public class VideoCropActivity extends AppCompatActivity implements VideoPlayer.OnProgressUpdateListener, VideoSliceSeekBarH.SeekBarChangeListener {
    ...
    public static Intent createIntent(Context context, String inputPath, String outputPath) {
        Intent intent = new Intent(context, com.hhmd.videocrop.VideoCropActivity.class);
        ...
        switch (requestCode) {
            case STORAGE_REQUEST: {
                if (grantResults.length > 0
                    ...
                    Toast.makeText(this, "您必须授予写存储权限才能使用此功能", Toast.LENGTH_SHORT).show();
                    setResult(RESULT_CANCELED);
                    finish();
                }
            }
        ...
        if (mFFMpeg.isSupported()) {
            ...
            mFFTask = mFFMpeg.execute(cmd, new ExecuteBinaryResponseHandler() {
                ...
                public void onFailure(String message) {
                    mFFMpeg.deleteCrop(outputPath);
                    Toast.makeText(com.hhmd.videocrop.VideoCropActivity.this, "裁剪失败!", Toast.LENGTH_SHORT).show();
                ...
                public void onStart() {
                    ...
                    mTmbProgress.setVisibility(View.INVISIBLE);
